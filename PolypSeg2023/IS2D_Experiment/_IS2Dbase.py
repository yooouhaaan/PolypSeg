...
from dataset.PolypSeg import *
from dataset.ForestTypeSegmentationDataset import *
...
class BaseSegmentationExperiment(object):
    def __init__(self, args):
        ...
        self.metric_list = ['Loss', 'Confusion Matrix']
        ...
        if args.data_type in ['CVC-ClinicDB', 'Kvasir-SEG', 'BKAI-IGH-NeoPolyp'] :
            ...
            train_dataset = ForestImageSegmentationDataset(args.dataset_dir, mode='train', transform=self.transform_generator('train')[0], target_transform=self.transform_generator('train')[1])
            val_dataset = ForestImageSegmentationDataset(args.dataset_dir, mode='val', transform=self.transform_generator('test')[0], target_transform=self.transform_generator('test')[1])
            test_dataset = ForestImageSegmentationDataset(args.dataset_dir, mode='val', transform=self.transform_generator('test')[0], target_transform=self.transform_generator('test')[1])
        else:
            print("Wrong Dataset")
            sys.exit()
